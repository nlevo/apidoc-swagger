...
var argv = nomnom
    .option('useApidocJson', { flag: true, 'default': false, help: 'Skip reading apidoc annotations in the code, use a json file generated by apidoc package' })
    .option('apidocDataInput', { 'default': './', help: 'Path to json file generated by apidoc package' })
    .parse()
;
...
var defaults = {
    ...
    colorize: true,
    markdown: true,
    useApidocJson: argv.useApidocJson,
...
function createApidocSwagger(options) {
    ...
    try {
        ...
        if(app.options.useApidocJson) {
            api = JSON.parse(fs.readFileSync(argv.apidocDataInput, 'utf8'));
        } else {
        ...
        if (app.options.parse !== true) {
            var apidocData = app.options.useApidocJson ? api : JSON.parse(api.data);
            var projectData;
            if(app.options.useApidocJson) {
                projectData = JSON.parse(fs.readFileSync(argv.input, 'utf8'));
            } else {
                projectData = JSON.parse(api.project);
            }
